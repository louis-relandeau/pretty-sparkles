...
// Dependencies: GLFW, GLAD, ImGui (with backends: imgui_impl_glfw,
// imgui_impl_opengl3)
#include "imgui.h"
...
int main() {
  const std::filesystem::path configPath = root / "simulation.cfg";
  SimulationConfig currentConfig;

  auto buildBoundaryFromConfig = [&](BoundaryConditionGrid &outGrid) {
    currentConfig = loadSimulationConfig(configPath.string());

    std::unique_ptr<BoundaryGeometry> boundaryShape;
    if (currentConfig.shape == ShapeKind::Circle) {
      boundaryShape = std::make_unique<CircularShape>(
          currentConfig.circleThickness, currentConfig.circleCenterPotential,
          currentConfig.circleOuterPotential);
    } else {
      boundaryShape = std::make_unique<NeedlePlateShape>(
          currentConfig.needleLength, currentConfig.needleHalfWidth,
          currentConfig.needleTopOffset, currentConfig.needlePlateThickness,
          currentConfig.needlePotential, currentConfig.platePotential);
    }

    boundaryShape->build(outGrid, N);
    geometry = outGrid.renderMask;
  };

  BoundaryConditionGrid boundaryGrid;
  buildBoundaryFromConfig(boundaryGrid);

  SolverDBM solver(field_arr, arc, N, boundaryGrid.fixedMask,
                   boundaryGrid.fixedPotential);
  solver.init();
  ...
  GLFWwindow *window = glfwCreateWindow(WIN_W, WIN_H, "Matrix Heatmap + ImGui",
                                        nullptr, nullptr);
  if (!window) {
  ...
  Shader shader((root / "Shaders/vertexShader.glsl").string(),
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    ImGui::Combo("##cmap", &colormapIdx, colormapNames,
                 IM_ARRAYSIZE(colormapNames));
    if (ImGui::Button("RESET")) {
      buildBoundaryFromConfig(boundaryGrid);
      solver.setBoundaryConditions(boundaryGrid.fixedMask,
                                   boundaryGrid.fixedPotential);
      solver.init();
    }
    ImGui::TextWrapped("Config file: %s", configPath.string().c_str());
